...
import com.example.news.domain.repository.UpdateWifiOnlyUseCase
...
import com.example.news.domain.usecase.UpdateNotificationStatusUseCase
    ...
    fun processCommand(command: SettingsCommand) {
        when(command) {
            ...
            is SettingsCommand.SetNotificationsEnabled -> {
                viewModelScope.launch {
                    updateNotificationStatusUseCase(command.enabled)
                }
            }
